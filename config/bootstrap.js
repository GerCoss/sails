...
module.exports.bootstrap = async function(done) {
    ...
    var adminUser = await User.create({
    ...
    var gerCoss = await User.create({
        emailAddress: 'admin2@example.com',
        fullName: 'Gerardo Coss',
        password: await sails.helpers.passwords.hashPassword('abc123')
    }).fetch();

    await User.addToCollection(gerCoss.id, 'friends', adminUser.id);

    //Create some things
    await Thing.createEach([
        { label: 'taladro', owner: adminUser.id },
        { label: 'bici', owner: gerCoss.id },
        { label: 'cd', owner: gerCoss.id },
    ]);
